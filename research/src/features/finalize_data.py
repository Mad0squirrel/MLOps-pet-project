...
def load_amenity_data(amenity_files: list[str]) -> dict[str, list[dict[str, float]]]:
    ...
    Parameters
    ----------
     amenity_files: list[str]
    
    Returns
    -------
     dict with amenity as key and list coordinates as value: dict[str, list[dict[str, float]]]
            ...
            for item in amenity_data[amenity]:
                ...
                for distance in DISTANCES:
                    if calculated_distance < distance:
                        key = amenity + "_" + str(distance)
                        coordinates_df.at[index, key] = coordinates_df.iloc[index][key] + 1
    # merge data
